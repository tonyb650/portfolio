...
const recordUserDetailsTool = tool({
  description:
    'Use this tool to record that a user is interested in being in touch and has provided an email address',
  ...
  execute: async ({
    ...
    return status === 1
      ? 'Successfully recorded user details and will contact the user soon.'
      : 'Because of technical errors, could not successfully record the user details.'
  },
})

const recordUnknownQuestionTool = tool({
  description:
    "Always use this tool to record any question that couldn't be answered because you did not know the answer",
  inputSchema: z.object({
    question: z.string().describe("The question that couldn't be answered"),
  }),
  execute: async ({ question }) => {
    const status = await sendPush(
      `Received a question that couldn't be answered: ${question}`
    )
    return status === 1
      ? 'Successfully recorded the question that could not be answered.'
      : 'Because of technical errors, could not successfully record the question.'
  },
})
